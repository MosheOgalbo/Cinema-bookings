...
function showSeatDetails(pos) {
    ...
    const elHighlightBtn = elPopup.querySelector('.btn-highlight')
    ...
    for (var i = rowIdx - 1; i <= rowIdx + 1; i++) {
        ...
        for (var j = colIdx - 1; j <= colIdx + 1; j++) {
            ...
            if (currCell.isSeat && !currCell.isBooked) {
                const elNeg = document.querySelector(`[data-i="${i}"][data-j="${j}"]`)
                console.log('elNeigh:', elNeg)
                elNeg.classList.add('highlight')
                setTimeout(() => elNeg.classList.remove('highlight') , 2500)
            }
        }
    }
}
